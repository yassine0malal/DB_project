...
export const ProfileHeader = ({ user, isOwnProfile }: { user: User; isOwnProfile: boolean }) => {
    ...
    const handleMessageClick = async () => {
        ...
        const discussionId = await startChat(currentUser.id, user.id);
        if (discussionId) {
            navigate('/messages');
        }
    };
                        ...
                        <div className="flex gap-3 mt-6 md:mt-0 pb-2">
                            {isOwnProfile ? (
                                ...
                                <>
                                    ...
                                    <Button
                                        variant="outline"
                                        className="gap-2 px-5 rounded-xl dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all"
