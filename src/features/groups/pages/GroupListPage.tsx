...
export const GroupListPage = () => {
    const { groups, filters, setFilter, getFilteredGroups, setGroups } = useGroupStore();
    const [isCreateOpen, setIsCreateOpen] = useState(false);
    const filteredGroups = getFilteredGroups();

    useEffect(() => {
        // Load mock groups if store is empty
        if (groups.length === 0) {
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Créez et rejoignez des groupes pour collaborer et échanger.</p>
                </div>
                <Button
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {groupTypeFilters.slice(1).map(({ value, label, icon: Icon }) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`p-2 rounded-lg ${value === 'friends' ? 'bg-pink-100 text-pink-600' :
                                    value === 'apartment' ? 'bg-amber-100 text-amber-600' :
                                        value === 'class' ? 'bg-blue-100 text-blue-600' :
            ...
            {filteredGroups.length > 0 ? (
                ...
                <div className="text-center py-20 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                    ...
                    <p className="text-gray-500 mt-1">Essayez de modifier vos filtres ou créez un nouveau groupe.</p>
                    <Button
                        onClick={() => setIsCreateOpen(true)}
