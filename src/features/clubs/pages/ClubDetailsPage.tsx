...
export const ClubDetailsPage = () => {
    ...
    const { clubs, joinClub, leaveClub } = useClubStore();
    ...
    if (!club) {
        return (
            <div className="text-center py-20">
                <h2 className="text-2xl font-bold text-gray-900">Club introuvable</h2>
                <Link to="/clubs">
                    <Button variant="link" className="text-blue-600">Retourner à la liste</Button>
                </Link>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-4xl mx-auto pb-10 space-y-6">
            ...
            {/* Header */}
            <div className="relative h-64 w-full rounded-2xl overflow-hidden bg-gray-200 shadow-md">
            ...
            <div className="flex gap-1 border-b border-gray-200">
                {(['feed', 'events', 'members', 'about'] as Tab[]).map((tab) => (
                    <button
                        ...
                        className={cn(
                            "px-6 py-3 text-sm font-medium text-gray-500 border-b-2 border-transparent hover:text-gray-900 transition-colors capitalize",
            ...
            <div className="min-h-[300px]">
                {activeTab === 'feed' && (
                    <div className="space-y-6">
                        {isMember && (
                            <Card className="p-4 border-dashed border-2 shadow-none bg-gray-50 flex items-center justify-center cursor-pointer hover:bg-gray-100 transition-colors">
                ...
                {activeTab === 'events' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {club.events.length > 0 ? (
                            ...
                            <div className="text-center py-10 text-gray-500 col-span-full">Aucun événement prévu.</div>
                        )}
                    </div>
                )}
                ...
                {activeTab === 'members' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {club.members.map(member => (
                            <Card key={member.user.id} className="p-4 flex items-center justify-between group hover:shadow-md transition-all border-gray-100">
                                <div className="flex items-center gap-4">
                                    <div className="relative">
                                        ...
                                        {member.role === 'admin' && (
                                            <div className="absolute -top-1 -right-1 bg-amber-400 text-white rounded-full p-1 shadow-sm border border-white">
                                                <Shield className="h-3 w-3" />
                                            </div>
                                        )}
                                        {member.role === 'moderator' && (
                                            <div className="absolute -top-1 -right-1 bg-blue-500 text-white rounded-full p-1 shadow-sm border border-white">
                    ...
                    <div className="space-y-8">
                        ...
                        {(club.contactEmail || club.website) && (
                            <div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {club.contactEmail && (
                                        <Card className="p-4 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                                            <div className="h-10 w-10 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600">
                                                <Mail className="h-5 w-5" />
                                            </div>
                                            <div>
                                                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Email</p>
                                                <p className="text-sm font-semibold text-gray-700">{club.contactEmail}</p>
                                            </div>
                                        </Card>
                                    )}
                                    {club.website && (
                                        <a href={club.website} target="_blank" rel="noopener noreferrer">
                                            <Card className="p-4 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                                                ...
                                                <div>
                                                    <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Site Web</p>
                                    ...
                                    <ul className="space-y-3">
                                        {club.rules.map((rule, i) => (
                                            <li key={i} className="flex gap-3 text-gray-700">
                                                <span className="flex-shrink-0 h-6 w-6 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center text-xs font-bold">
                                                    {i + 1}
                                                </span>
